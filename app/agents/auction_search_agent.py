...
class HeuristicAuctionSearchEngine(AuctionSearchEngine):
    def search(self, listings: list[Listing], search_config: SearchConfig) -> AuctionSearchResult:
        decisions = [
            AuctionSearchDecision(
                ...
                should_track=True,
                confidence=0.55,
                rationale="Selected by fallback auction-search engine for market analysis.",
            )
            for listing in listings
        ]
        return AuctionSearchResult(decisions=decisions)
